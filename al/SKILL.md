...
6. 图片：`render_pdf.py` 渲染的页面是**给 Agent 自己读题干、对答案用的**，用户手上有原书和做题本，报题号即可，别推给用户。要送到用户眼前的是 Agent 自己画的讲解配图 —— 宿主的读图工具不一定向用户展示，**默认按用户看不见处理**，先读图自查再显式推送（Claude Code / Desktop 用 `SendUserFile`）。**正文里不要写 `![...](...)` 引用本地图片**（Web 端加载不了）。细则见 PROTOCOL「图片：两种用途，别混」。
7. 项目侧约定优先：课程目录或其上级若有 `AGENTS.md` / `CLAUDE.md` 等项目约定文件（讲解深度、画图规范、素材来源等），**开工前先读**，与本 skill 冲突时以项目约定为准。宿主不一定自动把它加载进上下文。
